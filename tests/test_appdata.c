...
void setUp() {
    if (data_dir_exists()) {
        ...
        if (data != NULL) {
            FILE *f = fopen("backup_api_key.txt", "w");
            fputs(data->api_key, f);
            fclose(f);
            char file_path[257];
            char *home = getenv("HOME");
            snprintf(file_path, 257, "%s/%s", home, ".cgptchatbot");
            strcat(file_path, "/");
...
void tearDown() {
    if (data_dir_exists()) {
        ...
        FILE *f = fopen("backup_api_key.txt", "r");
        ...
        if (f != NULL) {
            ...
            APPDATA *data = save_api_key_and_create_data(api_key);
